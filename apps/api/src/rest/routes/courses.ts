...
const allCoursesRoute = createRoute({
  ...
  responses: {
    200: {
      content: { "application/json": { schema: responseSchema(courseSchema.array()) } },
...
coursesRouter.openapi(allCoursesRoute, async (c) => {
  ...
  return c.json({ ok: true, data: courseSchema.array().parse(await service.getAllCourses()) }, 200);
});
...
coursesRouter.openapi(coursesByFiltersRoute, async (c) => {
  ...
  const service = new CoursesService(database(c.env.DB.connectionString));
  return c.json(
    { ok: true, data: courseSchema.array().parse(await service.getCourses(query)) },
    200,
  );
});
